...
const EventContainer = ({ searchTerm, route }: { searchTerm: string; route: string }) => {
  ...
  const filteredData = data?.filter((event: EventCardPropType) => event.title.toLowerCase().includes(searchTerm.toLowerCase().trim()) || event.location.toLowerCase().includes(searchTerm.toLowerCase().trim()) || event.date.includes(searchTerm.toLowerCase().trim()) || event.time.includes(searchTerm.toLowerCase().trim()));
