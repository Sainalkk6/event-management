...
const EventDetails = ({ ...props }: EventCardPropType) => {
  ...
  const unbookEvent = async () => {
    await fetch(`/api/booking/unbook-event/${props.id}`, {
      method: "DELETE",
    });
    router.push("/");
  };
  ...
  return (
    <div className="flex w-full bg-[#fff] font-poppins flex-col items-center justify-center overflow-hidden">
      {showModal && <div className="flex min-h-[100vh]  top-0 w-full items-center justify-center bg-black bg-opacity-80 absolute">{data?.user?.name === "" || data?.user === undefined ? <LoginMessageContainer ref={ref} setShowModal={setShowModal} /> : <SeatCountContainer ref={ref} setShowModal={setShowModal} id={props.id} count={count} setCount={setCount} />}</div>}
      <div className="flex max-w-[1140px] flex-col">
        ...
        <div className="flex py-5 px-4 gap-[10px] flex-col">
          <div className="flex items-center py-4 justify-center gap-[10px] border-b border-[#dbdbdb]">
            ...
            <button
              onClick={() => {
                pathname.includes("my-bookings") ? unbookEvent() : setShowModal(true);
              }}
            ...
            >
              {pathname.includes("my-bookings") ? "Cancel" : "Book"}
            </button>
          </div>
